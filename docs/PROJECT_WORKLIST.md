...
- [!] **v3.0.3: 12.17 ICE/STUN augmentation — RETRACTED
  2026-05-23 by v2.5 Nebula-fabric lock.** Nebula's UDP
  hole-punching is protocol-level; the STUN module
  `crates/mackesd/src/stun.rs` + its 13 unit tests delete in
  NF-4.5. The TransportRegistry block that flipped this to
  `[!]` on 2026-05-23 is moot — the registry surface itself
  shrinks under NF-4.x. Original entry below preserved for
  audit only.
  Original: shipped 2026-05-20. New module
  ...
  `timeout` arg.
- [!] **v3.0.3: 12.18 HTTPS-tunneled fallback — RETRACTED
  2026-05-23 by v2.5 Nebula-fabric lock.** The Q10 covert
  transport remains a locked design requirement, but the
  implementation moves to the NF-1.x workstream. The
  activation state machine + 20 unit tests in
  `crates/mackesd/src/https_fallback.rs` migrate into
  `crates/mackes-nebula-https-tunnel/src/activation.rs`
  (NF-1.4); the wire-protocol layer (rustls TLS 1.3 over TCP/443,
  4-byte length-prefixed framing, byte-indistinguishable from
  HTTP/2 long-poll) is net-new code under NF-1.2 + NF-1.3.
  No parallel transport survives — Nebula is the only fabric
  and the TCP/443 path wraps its UDP frames. Original entry
  below preserved for audit only.
  Original: shipped
