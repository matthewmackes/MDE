...
//! [`check_plugin_allowed`] call before the host forwards it to
//! the matching plugin handler. The v2.1 KDC2 security lock
//! denies `run_command` by default; operators opt in via
//! `/etc/mde/connect/policy.toml` (KDC2-1.10) or the user
//! override at `~/.config/mde/connect/policy.toml`.
//!
//! The authority is a thin trait so mackesd's full
//! `LoadedPolicy` (which holds scorer + plugin lists)
//! satisfies it without dragging mackesd into this crate's
//! dep tree.

use mde_kdc_proto::plugins::PluginKind;

/// Outcome of a single dispatch check. Stable Display tokens
/// for audit-log entries.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DispatchDecision {
    /// Policy permits this plugin to act on behalf of this peer.
    /// Host forwards the packet to the plugin handler.
    Allowed,
    /// Policy denies the plugin. Host drops the packet + writes
    /// an audit entry. The optional `reason` token surfaces in
    /// the audit log for operator-side debugging.
    Denied {
...
impl DispatchDecision {
    ...
    pub const fn is_allowed(&self) -> bool {
        matches!(self, DispatchDecision::Allowed)
    }
}
...
impl std::fmt::Display for DispatchDecision {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DispatchDecision::Allowed => f.write_str("allowed"),
            DispatchDecision::Denied { reason } => write!(f, "denied({reason})"),
        }
    }
}

/// Decision authority. Implementations expose the plugin
/// allow/deny lists. mackesd's `transport::policy::LoadedPolicy`
/// is the production impl; tests use the inline `FixedPolicy`
/// helper below.
pub trait PluginAuthority {
    /// True when the plugin (by its packet-kind token, e.g.
    /// `"clipboard"`, `"run_command"`) is allowed by the
...
/// packet from `peer_id`. `paired` should be true when the
/// peer is currently in the pairing store. Unpaired peers can
/// never trigger a privileged plugin (defense in depth).
///
/// Pure function — no audit-log side-effect; the caller emits
/// the audit entry from the return value's Display rendering.
pub fn check_plugin_allowed(
    ...
    // Unpaired peers can talk via ping / clipboard / share /
    // notification only — privileged plugins (RunCommand, SMS)
    // require pairing.
...
/// Extension on `DispatchDecision` for the audit-log hint.
/// Currently a no-op `.also_log` that's a placeholder for the
/// future audit-chain wire-up (KDC2-3.11.b).
...
mod tests {
    ...
    /// Tiny test fixture implementing PluginAuthority. Real
    /// callers use mackesd's `LoadedPolicy`.
    struct FixedPolicy {
    ...
    fn clipboard_allowed_for_unpaired_peer_because_not_privileged() {
        // Pings + clipboards from unpaired peers are OK — they
        // can't elevate, just exchange small data. (Pairing
        // gates the *initial* exchange at the TLS handshake
        // layer; this dispatch-level check is defense in depth.)
        let policy = FixedPolicy {
            allow: vec![],
            deny: vec![],
        };
